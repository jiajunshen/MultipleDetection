...
def encoder_network(x, weights, bias, weights_key, bias_key):
    ...
    z_mean = tf.add(tf.matmul(encoder_pool_3_reshape, weights[weights_key[3]]), bias[bias_key[3]])
    z_log_sigma_sq = dense_layer(encoder_pool_3_reshape, weights[weights_key[4]], bias[bias_key[4]])
...
def decoder_network(x, weights, bias, weights_key, bias_key):
    ...
    decoder_conv_3 = conv2d(decoder_upscale_3, weights[weights_key[6]], bias[bias_key[6]])
    decoder_upscale_2 = upscale2d(decoder_conv_3, [1, 2], scale = 2)
    decoder_conv_2 = conv2d(decoder_upscale_2, weights[weights_key[7]], bias[bias_key[7]])
    decoder_upscale_1 = upscale2d(decoder_conv_2, [1, 2], scale = 2)
    decoder_conv_1 = conv2d(decoder_upscale_1, weights[weights_key[8]], bias[bias_key[8]], activation = tf.nn.sigmoid)
...
def main():
    ...
    weights = {
        'encode_conv_weight_1':tf.Variable(tf.random_uniform((5, 5, 3, 16), minval = -boundary, maxval = boundary)),
        'encode_conv_weight_2':tf.Variable(tf.random_uniform((5, 5, 16, 32), minval = -boundary, maxval = boundary)),
        'encode_conv_weight_3':tf.Variable(tf.random_uniform((5, 5, 32, 64), minval = -boundary, maxval = boundary)),
        ...
        'decode_conv_weight_3': tf.Variable(tf.random_uniform((5, 5, 64, 32), minval = -boundary, maxval = boundary)),
        'decode_conv_weight_2': tf.Variable(tf.random_uniform((5, 5, 32, 16), minval = -boundary, maxval = boundary)),
        'decode_conv_weight_1': tf.Variable(tf.random_uniform((5, 5, 16, 3), minval = -boundary, maxval = boundary))
    }

    bias = {
        'encode_conv_bias_1':tf.Variable(tf.zeros([16])),
        'encode_conv_bias_2':tf.Variable(tf.zeros([32])),
        ...
        'decode_conv_bias_3': tf.Variable(tf.zeros([32])),
