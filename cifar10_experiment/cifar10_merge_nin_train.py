...
tf.app.flags.DEFINE_string('train_dir', '/home-nfs/jiajun/Research/MultipleDetection/cifar10_experiment/cifar10_theano_train_merge_nin',
...
tf.app.flags.DEFINE_string('load_model', '/home-nfs/jiajun/Research/MultipleDetection/cifar10_experiment/cifar10_theano_train_Nov_18/model.npy',
...
def train():
    ...
    cnn_model, cnn_mid_output, weight_decay_penalty = cifar10_merge_nin.build_cnn(image_input_var)
    ...
    rotated_cnn_model, rotated_model_mid, rotated_weight_penalty = \
        cifar10_merge_nin.build_cnn(rotated_image_input_var, rotated_net = True, all_weights = all_weights)

    cnn_model_val, _, _ = cifar10_merge_nin.build_cnn(rotated_image_input_var)    
    ...
    if os.path.isfile(os.path.join(FLAGS.train_dir, 'latest_model.txt')):
        ...
        rotated_model_weights = np.load(weight_file)
        lasagne.layers.set_all_param_values(rotated_cnn_model, rotated_model_weights)
    ...
    for step in xrange(FLAGS.max_steps):
        ...
        if step % 50000 == 0 or (step + 1) == FLAGS.max_steps:
