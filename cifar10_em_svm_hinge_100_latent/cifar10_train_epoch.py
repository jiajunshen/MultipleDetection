...
def train():
    ...
    cifar10_data.train._cached_deformation = np.array(np.zeros((cifar10_data.train.shape[0], 10, 2 * 16)),dtype = np.float32)

    X_test = cifar10_data.sumple_test
    ...
    for epoch in xrange(max_epochs):
        ...
        if epoch % 50 == 0 or epoch + 1 == num_epochs:
            ...
            X_test._cached_deformation = np.array(np.zeros((X_test.shape[0], 10, 2 * 16)), dtype = np.float32)
        ...
        while(test_image is not None):
            ...
            for i in range(200):
                weightsOfParams = lasagne.layers.get_all_param_value(cnn_model)
        ...
        if epoch % 500 == 0 or (step + 1) == max_epochs:
