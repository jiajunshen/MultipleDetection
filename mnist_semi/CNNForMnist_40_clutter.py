...
def add_clutter(inputs, images, mega_patch_w=8, size=40, num_strokes=5):
    if len(inputs.shape) == 3:
        inputs = inputs.reshape(inputs.shape[0], 1, inputs.shape[1], inputs.shape[2])

    image_size, image_channel, image_w, image_h = images.shape

    cluttered_image = np.zeros((inputs.shape[0], 1, size, size), dtype = np.float32)
    ...
    for i in range(inputs.shape[0]):
        for j in range(num_strokes):
            ...
            cluttered_image[i, 0, t_x:t_x + mega_patch_w, t_y:t_y + mega_patch_w] = patch

        x_value_index = np.where(np.sum(input_image[0], axis = 1) != 0)[0]
        y_value_index = np.where(np.sum(input_image[0], axis = 0) != 0)[0]
        ...
        margin_size_x = (size - x_length) // 2
        margin_size_y = (size - y_length) // 2
        cluttered_image[i, :, margin_size_x : margin_size_x + x_length, margin_size_y : margin_size_y + y_length] = inputs[i, :, x_left: x_left + x_length, y_left: y_left + y_length]
    return cluttered_image
...
def main(model='mlp', num_epochs=500):
    ...
    _, _, X_test_rotated, y_test_rotated = load_data("/X_train.npy", "/Y_train.npy", "/X_test_cluttered.npy", "/Y_test_cluttered.npy")
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, y_train, 100, shuffle=True):
            ...
            cluttered_inputs = add_clutter(inputs[i], X_train)
            train_err += train_fn(cluttered_inputs, targets)
            train_batches += 1
    ...
    np.save("../data/mnist_Chi_dec_100_test_clutter.npy", weightsOfParams)
