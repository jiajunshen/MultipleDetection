...
def function_wrapper_8(input_data):
    return T.nnet.abstract_conv.bilinear_upsampling(input_data, 8)
...
def build_cnn(input_var=None):
    ...
    norm3 = BatchNormLayer(conv3a)

    pool3 = MaxPool2DLayer(norm3, pool_size=(2, 2), stride=(2, 2), pad=0)
    
    conv4 = Conv2DLayer(lasagne.layers.dropout(pool3, p = 0.5),
                        num_filters=256, filter_size=(3,3),
                        nonlinearity=lasagne.nonlinearities.rectify,
                        pad='same', W=lasagne.init.GlorotUniform(),
                        b=lasagne.init.Constant(0.1),
    ...
    norm4 = BatchNormLayer(conv4)

    pool4 = MaxPool2DLayer(norm4, pool_size=(2, 2), stride=(2, 2), pad=0)


    # fc1
    fc1 = DenseLayer(lasagne.layers.dropout(pool4, p = 0.5),
                     num_units=256,
                     nonlinearity=lasagne.nonlinearities.rectify,
                     W=lasagne.init.GlorotUniform(), b=lasagne.init.Constant(0.1),
                     name="fc1")

    conv2_upscale=lasagne.layers.ExpressionLayer(conv2, function_wrapper_2, output_shape=(None, 128, 96, 96))
    conv2a_upscale=lasagne.layers.ExpressionLayer(conv2a, function_wrapper_2, output_shape=(None, 128, 96, 96))
    conv3_upscale=lasagne.layers.ExpressionLayer(conv3, function_wrapper_4, output_shape=(None, 256, 96, 96))
    conv3a_upscale=lasagne.layers.ExpressionLayer(conv3a, function_wrapper_4, output_shape=(None, 256, 96, 96))
    conv4_upscale=lasagne.layers.ExpressionLayer(conv4, function_wrapper_8, output_shape=(None, 256, 96, 96))
    norm2_upscale=lasagne.layers.ExpressionLayer(norm2, function_wrapper_2, output_shape=(None, 128, 96, 96))
    norm3_upscale=lasagne.layers.ExpressionLayer(norm3, function_wrapper_4, output_shape=(None, 256, 96, 96))
    norm4_upscale=lasagne.layers.ExpressionLayer(norm4, function_wrapper_8, output_shape=(None, 256, 96, 96))
