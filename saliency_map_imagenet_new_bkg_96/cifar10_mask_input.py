...
def read_data_sets(data_dir, distortion=True, dtype=np.float32, training_num = 180000):
    ...
    train_image = np.array(np.load(os.path.join(data_dir, "real_background_large_10_class_train_newbkg.npy")).reshape(-1, 96, 96, 3), dtype=dtype)
    train_image = np.rollaxis(train_image, 3, 1)
    train_labels = np.array(np.load(os.path.join(data_dir, "mask_large_10_class_train.npy")).reshape(-1, 96 * 96),dtype=dtype) > 0
    ...
    test_image = np.array(np.load(os.path.join(data_dir, "real_background_large_10_class_test_newbkg.npy")).reshape(-1, 96, 96, 3), dtype=dtype)

    
    test_image = np.rollaxis(test_image, 3, 1)
    test_image = per_image_standardization(test_image)
    test_labels = np.array(np.load(os.path.join(data_dir, "mask_large_10_class_test.npy")).reshape(-1, 96 * 96), dtype=dtype) > 0
