...
def main(n=5, num_epochs=200, model=None):
    ...
    if model is None:
        ...
        #updates = lasagne.updates.adagrad(loss, params, learning_rate=sh_lr)
    ...
    if model is None:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in iterate_minibatches(X_train, Y_train, 128, shuffle=True, augment=False):
                ...
                train_batch_err, prediction = train_fn(inputs, targets)
                train_err += train_batch_err
                train_batches += 1
                if(train_batches == 1):
                    print(prediction.reshape(-1, 10)[:5])
        ...
        np.savez('cifar10_deep_residual_model_3_block.npz', *lasagne.layers.get_all_param_values(network))
    else:
