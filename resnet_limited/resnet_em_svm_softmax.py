...
def main(n=5, num_epochs=82, model=None):
    ...
    #network_selected = lasagne.layers.NonlinearityLayer(network_selected, nonlinearity=lasagne.nonlinearities.softmax)
    ...
    if model is None:
        ...
        prediction_selected = lasagne.nonlinearities.softmax(prediction_selected)

        with np.load("./cifar10_deep_residual_model_softmax.npz") as f:
        ...
        loss = lasagne.objectives.categorical_crossentropy(prediction_selected, target_var)
        loss = loss.mean()
        # add weight decay
    ...
    test_prediction_selected = lasagne.nonlinearities.softmax(test_prediction_selected)

    test_loss = lasagne.objectives.categorical_crossentropy(test_prediction_selected,
                                                            target_var)
    test_loss = test_loss.mean()
    test_acc = T.mean(T.eq(T.argmax(test_prediction_selected, axis=1), target_var),
                      dtype=theano.config.floatX)
    
    loss_affine_before = lasagne.objectives.squared_error(prediction_selected.clip(-20, 20), 20)
