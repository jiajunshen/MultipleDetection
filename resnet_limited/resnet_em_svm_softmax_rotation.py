...
def main(n=5, num_epochs=82, model=None):
    ...
    if model is None:
        ...
        prediction_classification = lasagne.nonlinearities.softmax(prediction_selected)

        loss_before = lasagne.objectives.multiclass_hinge_loss(prediction_classification, target_var)
    ...
    test_prediction_classification = lasagne.nonlinearities.softmax(test_prediction_selected)

    test_loss = lasagne.objectives.multiclass_hinge_loss(test_prediction_classification,
                                                            target_var)
    test_loss = test_loss.mean()
    test_acc = T.mean(T.eq(T.argmax(test_prediction_classification, axis=1), target_var),
