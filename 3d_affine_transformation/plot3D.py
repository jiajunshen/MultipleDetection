...
def plot_voxelgridnew(v_grid_vector, cmap="Oranges", axis=False):

    # coordinates returned from argwhere are inversed so use [:, ::-1]
    points = np.argwhere(v_grid_vector)[:, ::-1]

    s_m = plt.cm.ScalarMappable(cmap=cmap)
    rgb = s_m.to_rgba(v_grid_vector.reshape(-1)[v_grid_vector.reshape(-1) > 0])[:,:-1]

    camera_position = [-200, 100, 100]
    look = [100, 100, 100]
    ...
    with open("plotVG_new.html", "w") as html:
        html.write(TEMPLATE_VG.format(
            camera_x=camera_position[0],
            camera_y=camera_position[1],
            camera_z=camera_position[2],
            ...
            S_y=1,
            S_z=1,
            n_voxels=sum(v_grid_vector.reshape(-1) > 0),
            axis_size=axis_size))

    return IFrame("plotVG_new.html",width=800, height=800)
