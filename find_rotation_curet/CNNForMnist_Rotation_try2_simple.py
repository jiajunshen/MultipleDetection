...
def build_cnn(input_var=None, batch_size = None):
    ...
    network = Conv2DLayer(
            network, num_filters=32, filter_size=(11, 11),
            stride=(4,4),
...
def main(model='mlp', num_epochs=500):
    ...
    #saved_weights = np.load("../data/curet_test_hinge_epoch_100_2pool_61_classes_small.npy")
    ...
    final_test_prediction_res = T.max(test_prediction, 0)
    ...
    train_fn = theano.function([input_var, vanilla_target_var], [loss, test_prediction_res, T.eq(T.argmax(test_prediction_res, axis=1), vanilla_target_var)], updates=updates)

    # Compile a second function computing the validation loss and accuracy:
    val_fn = theano.function([input_var, vanilla_target_var], [test_loss, test_acc, test_prediction_res, T.eq(T.argmax(test_prediction_res, axis=1), vanilla_target_var)])
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, y_train, 100, shuffle=True):
            ...
            inputs = inputs.reshape(100, 227, 227)
            inputs = rotateImage_batch(inputs, nRotation).reshape(100 * nRotation, 1, 227, 227)
            duplicated_targets = np.array([targets for i in range(nRotation)]).reshape(100 * nRotation,)
            train_batch_err, prediction_array, correct_array = train_fn(inputs, targets)
            if epoch % 5 == 0 and epoch > 10:
                prediction_array = prediction_array.reshape(-1, 61)
                wrong_prediction = prediction_array[correct_array == False]
                wrong_label = targets[correct_array == False]
                print("Wrong Prediction Training: ", wrong_prediction.shape)
                print("wrong activation Training:", np.array([wrong_prediction[i, wrong_label[i]] for i in range(wrong_label.shape[0])]))
                print("best activation Training: ", np.max(wrong_prediction, axis = 1))
        ...
        if epoch % 10 == 0:
           # After training, we compute and print the test error:
            ...
            for batch in iterate_minibatches(X_test, y_test, 100, shuffle=False):
                ...
                inputs = inputs.reshape(100, 227, 227)
                inputs = rotateImage_batch(inputs, nRotation).reshape(100 * nRotation, 1, 227, 227)
                err, acc, prediction_array, correct_array = val_fn(inputs, targets)
                prediction_array = prediction_array.reshape(-1, 61)
                wrong_prediction = prediction_array[correct_array == False]
                wrong_label = targets[correct_array == False]
                print("Wrong Prediction: ", wrong_prediction.shape)
                print("wrong activation:", np.array([wrong_prediction[i, wrong_label[i]] for i in range(wrong_label.shape[0])]))
                print("best activation: ", np.max(wrong_prediction, axis = 1))
            ...
            for batch in iterate_minibatches(X_train, y_train, 100, shuffle=False):
                ...
                inputs = inputs.reshape(100, 227, 227)
                inputs = rotateImage_batch(inputs, nRotation).reshape(100 * nRotation, 1, 227, 227)
                err, acc, _, _ = val_fn(inputs, targets)
                test_err += err
                test_acc += acc
                test_batches += 1
            print("Final results:")
    ...
    np.save("../data/curet_justRotation_try2_simple.npy", weightsOfParams)
