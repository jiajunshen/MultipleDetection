...
def main(model='mlp', num_epochs=61):
    # Load the dataset
    print("Loading data...")
    # num_per_class = 100
    ...
    affine_matrix_matrix = np.array(np.zeros((batch_size * 61,)), dtype = np.float32)
    ...
    predictions = T.reshape(predictions, (-1, 61, 61))
    
    predictions_rotation = lasagne.layers.get_output(network_for_rotation, deterministic = True)
    
    predictions_rotation_for_loss = lasagne.layers.get_output(network_for_rotation, deterministic = False)
    ...
    #updates_model = lasagne.updates.adagrad(loss, model_params, learning_rate = 0.1)

    updates_model = lasagne.updates.nesterov_momentum(
             loss, model_params, learning_rate=0.001, momentum=0.9)

    test_prediction = lasagne.layers.get_output(network, deterministic=True)

    test_prediction = T.reshape(test_prediction, (-1, 61, 61))
    ...
    weightsOfParams = np.load("../data/Curet_CNN_Hinge_epoch_40_2pool_8rotation_400_epoch_wtf.npy")
    lasagne.layers.set_all_param_values(network, weightsOfParams)
    # Finally, launch the training loop.
    # We iterate over epochs:
    cached_affine_matrix = np.array(np.zeros((X_train.shape[0], 61,)), dtype = np.float32)
    for epoch in range(41, num_epochs):
        ...
        if epoch % 20 == 0 or epoch + 1 == num_epochs:
            ...
            if epoch != 0 and epoch % 20 == 0:
                X_test = X_test_all
                y_test = y_test_all
            cached_affine_matrix_test = np.array(np.zeros((X_test.shape[0], 61,)), dtype = np.float32)
            ...
            for batch in iterate_minibatches(X_test, y_test, batch_size, shuffle=False):
                ...
                for j in range(searchCandidate):
                    affine_params.set_value(np.array(np.ones(batch_size * 61) * eachDegree * j, dtype = np.float32))
                    for i in range(1):
                        weightsOfParams = lasagne.layers.get_all_param_values(network)
                        train_affine_res = train_affine_fn(inputs)
                        train_loss, train_loss_before = train_affine_res[:2]
                
                    affine_params_all.append(np.array(weightsOfParams[0].reshape(1, batch_size, 61)))
                    train_loss_before_all.append(train_loss_before.reshape(1, batch_size, 61))
                ...
                cached_affine_matrix_test[index] = affine_params_all_reshape.reshape(-1, 61)
        ...
        if epoch % 10 == 0:
            ...
            for batch in iterate_minibatches(X_train, y_train, batch_size, shuffle=False):
                ...
                cached_affine_matrix[index] = affine_params_all_reshape.reshape(-1, 61)
        ...
        if epoch % 10 == 0:
            weightsOfParams = lasagne.layers.get_all_param_values(network)
            np.save("../data/Curet_CNN_Hinge_epoch_%d_2pool_8rotation_400_epoch_wtf_every_10epoch_update_sbatch.npy" %epoch, weightsOfParams)
