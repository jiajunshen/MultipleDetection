...
def build_model(input_var=None):
    ...
    loc_l6 = lasagne.layers.DenseLayer(
        loc_l5, num_units=20, W=lasagne.init.HeUniform())
...
def main(model='mlp', num_epochs=100):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, y_train, 100, shuffle=True):
            ...
            angles = np.random.randint(low = -45, high = 45, size = 100)
            ...
            if train_batches == 1 and epoch % 5 == 0:
                np.save("./unrotated_image_train_cnn.npy", trans_img)
                np.save("./original_image_train_cnn.npy", rotated_inputs)
        ...
        if epoch % 5 == 0:
            ...
            for batch in iterate_minibatches(X_test, y_test, 500, shuffle=True):
                inputs, targets = batch
                angles = np.random.randint(low = -45, high = 45, size = 500)
                ...
                test_batches += 1
                if test_batches == 1:
                    np.save("./unrotated_image_cnn.npy", trans_img)
                    np.save("./original_image_cnn.npy", rotated_inputs)
            print("Final results:")
