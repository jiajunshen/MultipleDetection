...
def main(model='mlp', num_epochs=100):
    ...
    X_test = X_test_all[index[:60]]
    y_test = y_test[index[:60]]
    ...
    saved_weights = \
        np.load("../data/NIN_ModelNet40_hinge_epoch200.npy")

    affine_matrix_matrix = [np.zeros((batch_size * 40,), dtype=np.float32),
                            np.zeros((batch_size * 40,), dtype=np.float32),
                            np.zeros((batch_size * 40,), dtype=np.float32)]
    ...
    predictions = T.reshape(predictions, (-1, 40, 40))
    ...
    test_prediction = T.reshape(test_prediction, (-1, 40, 40))
    ...
    cached_affine_matrix = np.array(np.zeros((3, X_train.shape[0], 40)),
                                    dtype=np.float32)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch % 20 == 0 or epoch + 1 == num_epochs) and TEST:
            ...
            cached_affine_matrix_test = \
                np.array(np.zeros((3, X_test.shape[0], 40)), dtype=np.float32)
            ...
            for batch in iterate_minibatches(X_test, y_test,
                ...
                if Parallel_Search:
                    ...
                    for x in range(eachSearchCandidate):
                        x_degree = \
                            np.ones((batch_size * 40,), dtype=np.float32) * eachDegree * x - 45.0
                        for y in range(eachSearchCandidate):
                            y_degree = \
                                np.ones((batch_size * 40,), dtype=np.float32) * eachDegree * y - 45.0
                            for z in range(eachSearchCandidate):
                                z_degree = \
                                    np.ones((batch_size * 40,), dtype=np.float32) * eachDegree * z - 45.0
                    ...
                    searchCandidate = 200
                    affine_params[0].set_value(np.zeros((batch_size * 40,),
                                                        dtype=np.float32))
                    affine_params[1].set_value(np.zeros((batch_size * 40,),
                                                        dtype=np.float32))
                    affine_params[2].set_value(np.zeros((batch_size * 40,),
                                                        dtype=np.float32))

                    for i in range(searchCandidate):
                        weightsOfParams = lasagne.layers.get_all_param_values(network)
                        train_loss, train_loss_before = train_affine_fn(inputs)
                        affine_params_all.append(np.array(weightsOfParams[:3].reshape(1, 3, batch_size, 40)))
                        train_loss_before_all.append(train_loss_before.reshape(1, batch_size, 40))
                ...
                cached_affine_matrix_test[0, index] = affine_params_all_reshape_x.reshape(-1, 40)
                cached_affine_matrix_test[1, index] = affine_params_all_reshape_y.reshape(-1, 40)
                cached_affine_matrix_test[2, index] = affine_params_all_reshape_z.reshape(-1, 40)
        ...
        if epoch % 20 == 0:
            ...
            for batch in iterate_minibatches(X_train, y_train, batch_size, shuffle=False):
                ...
                if Parallel_Search:
                    ...
                    eachDegree = 90.0 / eachSearchCandidate
                    for x in range(eachSearchCandidate):
                        x_degree = np.ones((batch_size * 40,), dtype=np.float32) * eachDegree * x - 45.0
                        for y in range(eachSearchCandidate):
                            y_degree = np.ones((batch_size * 40,), dtype=np.float32) * eachDegree * y - 45.0
                            for z in range(eachSearchCandidate):
                                z_degree = np.ones((batch_size * 40,), dtype=np.float32) * eachDegree * z - 45.0
                                affine_params[0].set_value(x_degree)
                                affine_params[1].set_value(y_degree)
                                affine_params[2].set_value(z_degree)
                                for i in range(10):
                ...
                cached_affine_matrix[0, index] = affine_params_all_reshape_x.reshape(-1, 40)
                cached_affine_matrix[1, index] = affine_params_all_reshape_y.reshape(-1, 40)
                cached_affine_matrix[2, index] = affine_params_all_reshape_z.reshape(-1, 40)
        ...
        if epoch % 20 == 19:
            weightsOfParams = lasagne.layers.get_all_param_values(network)
            np.save("../data/NIN_ModelNet40_hinge_epoch_%d_3d_find_rotation.npy" % epoch, weightsOfParams)
