...
def iterate_minibatches_two(inputs, targets, inputs_real, targets_real, batchsize, shuffle=False):
    assert len(inputs) == len(targets)
    assert len(inputs_real) == len(targets_real)

    if shuffle:
        indices = np.arange(len(inputs))
        np.random.shuffle(indices)
    for start_idx in range(0, len(inputs) - batchsize//2 + 1, batchsize//2):
        if shuffle:
            excerpt = indices[start_idx:start_idx + batchsize//2]
        else:
            excerpt = slice(start_idx, start_idx + batchsize//2)
        random_index = np.random.randint(0, len(targets_real), batchsize//2)
        inputs_real_sample = inputs_real[random_index]
        targets_real_sample = targets_real[random_index]
        yield np.concatenate([inputs[excerpt], inputs_real_sample]), np.concatenate([targets[excerpt], targets_real_sample])
...
def main(model='mlp', num_epochs=500):
    ...
    X_train_real, y_train_real, _, _ = load_data("/imagenet_train.npy",
                                                 "/imagenet_train_label.npy",
                                                 #"/real_background_10_class_train.npy",
                                                 #"/10_class_train_label.npy",
                                                 "/imagenet_test.npy",
    ...
    X_train_real = np.vstack([X_train_real[i * total_num_per_class:i * total_num_per_class + train_num] for i in range(9)])
    y_train_real = np.concatenate([y_train_real[i * total_num_per_class : i * total_num_per_class + train_num] for i in range(9)])
    X_train = X_train
    y_train = y_train
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches_two(X_train, y_train, X_train_real, y_train_real, 100, shuffle=True):
