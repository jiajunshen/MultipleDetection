...
import cifar10
import cifar10_input_all_train as cifar10_input
...
train_dir = './cifar10_theano_train_adam_test_2'
max_epochs = 2000
validation = False
validation_model = ''
...
def train():
    ...
    hinge_loss = lasagne.objectives.multiclass_hinge_loss(model_deformation_for_loss, target_var, delta = 1)
    ...
    updates_model = lasagne.updates.adam(loss, model_params, learning_rate=0.01)
    ...
    for epoch in xrange(max_epochs):
        ...
        if 1: 
            ...
            while(test_image is not None):
                ...
                affine_params.set_value(np.zeros(batch_size * 10,))
        ...
        if 1:
            ...
            if epoch % 100 == 0:
                ...
                np.save("../data/CIFAR_CNN_params_drop_out_Chi_2017_ROT_hinge_2000_em_new_batch_run_epoch_%d.npy" %epoch, weightsOfParams)
