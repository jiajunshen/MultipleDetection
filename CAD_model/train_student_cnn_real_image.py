...
def main(model='mlp', num_epochs=1000):
    ...
    X_teacher_train, y_teacher_train, _, _ = load_data("/X_plain_real_train_rotation_100.npy",
                                                 "/Y_train_real_rotation.npy",
                                                 "/X_plain_test_100.npy",
                                                 "/Y_test.npy",
                                                 resize=False,
                                                 size = 100)
    X_student_train, y_student_train, X_student_test, y_student_test = \
        load_data("/X_texture_real_train_rotation_100.npy", "/Y_train_real_rotation.npy",
                  "/X_texture_real_test_100.npy", "/Y_test_real.npy",
    ...
    teacher_network, teacher_intermediate_layer = \
        build_teacher_cnn(teacher_input_var, int(sys.argv[1]))
    student_network, student_intermediate_layer = \
        build_student_cnn(student_input_var, int(sys.argv[1]))
    ...
    weightsOfTeacherNetwork = np.load("../data/plain_rotation_network_withoutBatchNorm_real_rotation.npy")
    #weightsOfTeacherNetwork = np.load("../data/plain_rotation_network_withoutBatchNorm.npy")

    teacher_weights = [weightsOfTeacherNetwork[i] for i in range(int(sys.argv[2]), 12)]

    network_saved_weights = [weightsOfIntermediateParams[i].eval() for i in range(int(sys.argv[2]))] + teacher_weights
