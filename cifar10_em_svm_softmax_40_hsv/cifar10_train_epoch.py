...
def train():
    ...
    categorical_loss = lasagne.objectives.categorical_crossentropy(model_output, target_var)

    categorical_loss = categorical_loss.mean()

    loss = categorical_loss + weight_decay_penalty

    params = lasagne.layers.get_all_params(cnn_model, trainable=True)

    updates = lasagne.updates.adam(loss, params, learning_rate=0.001)

    train_fn = theano.function([image_input_var, target_var], [loss, categorical_loss, weight_decay_penalty], updates=updates)

    test_acc = T.mean(T.eq(T.argmax(model_output_eval, axis = 1), target_var),
    ...
    for epoch in xrange(max_steps):
        ...
        if epoch % 1000 == 0 or (step + 1) == max_steps:
