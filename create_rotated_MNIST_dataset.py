...
X_test = extend_image(X_test, 60)
X_train = extend_image(X_train, 60)

train_size = y_train.shape[0]
all_images = []
all_labels = []
for j in range(1):
    angles_1 = list(np.random.randint(low = -20, high = -5, size = train_size // 2))
    angles_2 = list(np.random.randint(low = 5, high = 20, size = train_size // 2))
