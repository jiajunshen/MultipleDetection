...
import lasagne
from lasagne.layers import LocalResponseNormalization2DLayer, DenseLayer, Conv2DLayer, MaxPool2DLayer, InputLayer, DimshuffleLayer, BatchNormLayer, NonlinearityLayer
...
def build_cnn(input_var=None):
    ...
    # fc3
    fc2 = DenseLayer(lasagne.layers.dropout(fc1, p = 0.5),
                               num_units=10,
    ...
    softmax_layer = NonlinearityLayer(fc2, nonlinearity = lasagne.nonlinearities.softmax)

    # Weight Decay
    weight_decay_layers = {fc1: 0.0}
    l2_penalty = regularize_layer_params_weighted(weight_decay_layers, l2)
    network_middle_output = lasagne.layers.ReshapeLayer(fc2, shape = (([0], 10)))
