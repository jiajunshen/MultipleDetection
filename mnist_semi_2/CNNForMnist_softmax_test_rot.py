...
def main(model='mlp', num_epochs=2000):
    ...
    saved_weights = np.load("../data/mnist_CNN_params_drop_out_Chi_2017_all.npy")
    ...
    test_res = T.eq(T.argmax(test_prediction_res, axis=1), vanilla_target_var)
    # Compile a function performing a training step on a mini-batch (by giving
    # the updates dictionary) and returning the corresponding training loss:
    train_fn = theano.function([input_var, vanilla_target_var], loss, updates=updates)

    # Compile a second function computing the validation loss and accuracy:
    val_fn = theano.function([input_var, vanilla_target_var], [test_loss, test_acc, test_prediction_res, test_res])
   ...
   # After training, we compute and print the test error:
    ...
    for batch in iterate_minibatches(X_test, y_test, 500, shuffle=False):
        ...
        err, acc, prediction_res, test_output_res = val_fn(inputs, targets)
        test_err += err
        test_acc += acc
        test_batches += 1
        if test_batches == 1:
            print(targets[:100])
            print(prediction_res[:100])
            print(test_output_res[:100])
    print("Final results:")
