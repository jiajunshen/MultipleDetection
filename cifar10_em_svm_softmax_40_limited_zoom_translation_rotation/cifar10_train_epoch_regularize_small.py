...
def train():
    ...
    deformation_matrix_matrix = np.array(np.zeros((batch_size * 10, 4)), dtype = np.float32)
    deformation_matrix_matrix[:,2] = 1

    network_saved_weights = np.array([deformation_matrix_matrix[:,:2], ] + [deformation_matrix_matrix[:,2:3], ] + [deformation_matrix_matrix[:, 3:], ] + [saved_weights[i] for i in range(saved_weights.shape[0])])
    ...
    for param, grad in zip(affine_params, d_loss_wrt_params):
        ...
        if i == 2:
            #updates_affine[param] = param - 1.0 * theano.tensor.sgn(grad)
            updates_affine[param] = param - theano.tensor.clip(1.0 * grad, -5, 5)
        i += 1
    ...
    training_cached_deformation = np.array(np.zeros((X_train.shape[0], 10, 4)),dtype = np.float32)
    ...
    for epoch in xrange(max_epochs):
        ...
        if 1: 
            ...
            if epoch % 20 == 0 or epoch + 1 == max_epochs:
                ...
                testing_cached_deformation = np.array(np.zeros((X_current_test.shape[0], 10, 4)), dtype = np.float32)
                ...
                for batch in iterate_minibatches(X_current_test, Y_current_test, batch_size):
                    ...
                    set_value = np.array(np.zeros((batch_size * 10, 4)), dtype = np.float32)
                    set_value[:, 2] = 1
                    affine_params[0].set_value(set_value[:,:2])
                    affine_params[1].set_value(set_value[:, 2:3])
                    affine_params[2].set_value(set_value[:, 3:4])
                    for i in range(20):
                    ...
                    testing_cached_deformation[test_indices, :, 2:3] = weightsOfParams[1].reshape(-1, 10, 1)
                    testing_cached_deformation[test_indices, :, 3:4] = weightsOfParams[2].reshape(-1, 10, 1)
            ...
            for batch in iterate_minibatches(X_current_test, Y_current_test, batch_size):
                test_image, test_label, test_indices = batch
                set_value = testing_cached_deformation[test_indices].reshape(-1, 4)
                affine_params[0].set_value(set_value[:, :2])
                affine_params[1].set_value(set_value[:, 2:3])
                affine_params[2].set_value(set_value[:, 3:4])
            ...
            for batch in iterate_minibatches(X_train, Y_train, batch_size):
                ...
                for i in range(20):
                    ...
                    if epoch % 5 == 0 and affine_train_batches == 1:
                        ...
                        print("gradient of the degree: ", train_affine_result[6].reshape(-1, 10, 1)[0])
                        ...
                        print("Degree of each image", weightsOfParams[2].reshape(-1, 10, 1)[0])
                ...
                training_cached_deformation[train_indices, :, 2:3] = weightsOfParams[1].reshape(-1, 10, 1)
                training_cached_deformation[train_indices, :, 3:4] = weightsOfParams[2].reshape(-1, 10, 1)
                ...
                if affine_train_batches == 1 and epoch % 100 == 0:
                    ...
                    print(np.array([weightsOfParams[2].reshape(-1, 10, 1)[i, train_label[i], :] for i in range(batch_size)]))
        ...
        if 1:
            for batch in iterate_minibatches(X_train, Y_train, batch_size, shuffle = True):
                train_image, train_label, train_indices = batch
                set_value = training_cached_deformation[train_indices].reshape(-1,4)
                affine_params[0].set_value(set_value[:, :2])
                affine_params[1].set_value(set_value[:, 2:3])
                affine_params[2].set_value(set_value[:, 3:])
                train_loss_value, train_acc_value_1, train_acc_value_2 = train_fn(train_image, train_label)
