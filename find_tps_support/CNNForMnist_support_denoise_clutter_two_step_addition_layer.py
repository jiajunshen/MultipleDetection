    ...
    for epoch in range(num_epochs):
        ...
        if (epoch % 50 == 0 or epoch + 1 == num_epochs) and EvaluationSwitch:
            ...
            if UseSupportSwitch:
                ...
                np.save("./error_image.npy", np.vstack(error_image))
                np.save("./error_prediction.npy", np.concatenate(error_prediction))
                np.save("./error_original_image.npy", np.vstack(error_original_image))
                np.save("./error_transformed_image.npy", np.vstack(error_transformed_image))
        ...
        if epoch % 50 == 0:
            ...
            for batch in iterate_minibatches(X_train, y_train, batch_size, shuffle=False):
                ...
                for i in range(200):
                    weightsOfParams = lasagne.layers.get_all_param_values(network)
                    train_loss, train_loss_before, final_transformed_images, weight_decay = train_affine_fn(inputs)
                ...
                transformed_images_train_list.append(final_transformed_images)
        ...
        if epoch % 100 == 0: 
            weightsOfParams = lasagne.layers.get_all_param_values(network)
            np.save("../data/new_transformed_training_data_epoch%d" %epoch, transformed_images_list_support)
