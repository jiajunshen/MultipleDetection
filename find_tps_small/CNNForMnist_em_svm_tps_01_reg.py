...
def build_cnn(input_var=None, batch_size = None):
    ...
    network = Conv2DLayer(
            network_transformed_TPS, num_filters=4, filter_size=(5, 5),
    ...
    network = Conv2DLayer(
            network, num_filters=4, filter_size=(5, 5),
            nonlinearity=lasagne.nonlinearities.rectify,
            W = lasagne.init.GlorotUniform()
...
def main(model='mlp', num_epochs=2000):
    ...
    saved_weights = np.load("../data/mnist_CNN_params_drop_out_Chi_2017_hinge_simpler.npy")
    ...
    weightsOfParams = np.load("../data/mnist_CNN_params_drop_out_Chi_2017_Deformation_hinge_2000_script_run_reg_0_1_MNIST_em_new_epoch1999_simpler.npy")
    ...
    for epoch in range(previous_epoch, previous_epoch + num_epochs):
        ...
        if epoch % 50 == 0 or epoch + 1 == num_epochs + previous_epoch:
            print ("Start Evaluating...")
            test_acc = 0
            test_batches = 0
            affine_test_batches = 0
            if epoch + 1 == num_epochs or epoch % 500 == 0:
                X_test = X_test_all
                y_test = y_test_all
        ...
        if epoch % 500 == 0 or epoch == num_epochs + previous_epoch- 1: 
            weightsOfParams = lasagne.layers.get_all_param_values(network)
            np.save("../data/mnist_CNN_params_drop_out_Chi_2017_Deformation_hinge_2000_script_run_reg_0_1_MNIST_em_new_epoch%d_simpler.npy" %epoch, weightsOfParams)
