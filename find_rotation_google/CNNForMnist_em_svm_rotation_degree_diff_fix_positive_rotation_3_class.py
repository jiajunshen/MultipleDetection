...
def main(model='mlp', num_epochs=2000):
    ...
    predictions = T.reshape(predictions, (-1, 3))
    ...
    loss_before = lasagne.objectives.multiclass_hinge_loss(predictions, vanilla_target_var, 5)
    ...
    train_acc = T.mean(T.eq(T.argmax(predictions_rotation, axis = 1), vanilla_target_var), dtype = theano.config.floatX)
    ...
    for epoch in range(num_epochs):
        ...
        if 1:
            ...
            for batch in iterate_minibatches(X_test, y_test, batch_size, X_test_degree, shuffle=False):
                ...
                find_degree = -cached_affine_matrix_test[index][targets == 1][:, 1]%360
            ...
            final_find_car_degree = -cached_affine_matrix_test[all_target == 1][:, 1] % 360
            ...
            np.save("hard_cars.npy", valid_inputs[degree_diff_array >= 15])
        ...
        for batch in iterate_minibatches(X_train, y_train, batch_size, X_train_degree, shuffle=True):
            ...
            cached_affine_matrix[index[targets == 1], 1] = -correct_degree[targets == 1] % 360
            car_degree = correct_degree[targets == 1] % 360
            find_degree = -cached_affine_matrix[index][targets == 1][:, 1] % 360
            degree_diff_array = 180 - np.abs(180 - (car_degree - find_degree) % 360)
            #degree_diff_array[degree_diff_array > 90] = 180 - degree_diff_array[degree_diff_array > 90]
            degree_diff += np.sum(degree_diff_array)
        ...
        if 1:
            for batch in iterate_minibatches(X_train, y_train, batch_size, X_train_degree, shuffle=True):
                ...
                if train_batches == 1:
                    ...
                    print("find degree: ", -cached_affine_matrix[index][targets == 1][:, 1] % 360)
