...
def main(model='mlp', num_epochs=2000):
    ...
    loss = lasagne.objectives.multiclass_hinge_loss(predictions_rotation_for_loss, vanilla_target_var, 20)
    loss = loss.mean()
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 20 == 0:
            weightsOfParams = lasagne.layers.get_all_param_values(network)
            np.save("../data/google_earth_em_latent_svm_3_class_epoch%d_try_4.npy" %epoch, weightsOfParams)
