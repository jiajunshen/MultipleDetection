...
back_directory = "/hdd/Documents/Data/shapeNetTexture_bkg_10class/"
...
for item in items:
    ...
    for f in IKEA_texture_img_path:
        ...
        bkg_image = np.array(misc.imresize(bkg_image, (224, 224, 3)), dtype=np.uint8)
        ...
        image_directory = "/hdd/Documents/Data/shapeNetTexture_small_real_10class_newbkg/shapeNetTexture_small_realbackground_%s/" %(item)
        try:
            os.stat(image_directory)
        except:
            os.mkdir(image_directory) 

        misc.imsave("/hdd/Documents/Data/shapeNetTexture_small_real_10class_newbkg/shapeNetTexture_small_realbackground_%s/" %(item) + f.split("/")[6], new_image_final_f)
        count += 1
        if count  == 25000:
            break
