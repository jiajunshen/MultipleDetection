...
from rotationMatrixLayer_new import RotationMatrixLayer
...
def main(model='mlp', num_epochs=1):
    ...
    network, weight_decay, network_transformed, side_network = build_cnn(input_var, fake_input_var, batch_size)
    ...
    saved_weights = np.array([affine_matrix_matrix, affine_matrix_matrix] + [saved_weights[i] for i in range(saved_weights.shape[0])])
    ...
    predictions_affine = predictions[one_hot_targets.nonzero()].reshape((batch_size, 10))
    ...
    loss_affine_before = lasagne.objectives.squared_error(predictions_affine.clip(-20, 20), 20)
    ...
    test_prediction_affine = test_prediction[one_hot_targets.nonzero()].reshape((batch_size, 10))
    ...
    loss_test_affine = lasagne.objectives.squared_error(test_prediction_affine.clip(0, 20), 20)

    loss_test_affine = loss_test_affine.mean()

    update_affine_test = lasagne.updates.adagrad(loss_test_affine, affine_params, learning_rate = 1)
    
    # Compile a function performing a training step on a mini-batch (by giving
    # the updates dictionary) and returning the corresponding training loss:

    train_model_fn = theano.function([input_var, vanilla_target_var, target_var], loss, updates=updates_model)
    
    train_affine_fn = theano.function([input_var, target_var], [loss_affine, loss_affine_before, transformed_images, predictions, side_network_output], updates=updates_affine)
    
    eval_affine_fn = theano.function([input_var, target_var], [loss_affine, transformed_images, test_prediction, side_network_output])

    train_affine_fn_val = theano.function([input_var, target_var], [loss_test_affine, transformed_images, test_prediction], updates = update_affine_test)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, y_train, batch_size, shuffle=True):
            affine_params[0].set_value(affine_matrix_matrix)
            params[0].set_value(affine_matrix_matrix)
            ...
            if 1:
                for i in range(100):
                    train_loss, train_loss_before, transformed_image_res, predictions_res, side_network_result = train_affine_fn(inputs, fake_targets)
                    if i % 10 == -1:
                        print("Current Training Loss + max predition result")
                        print(train_loss, np.max(predictions_res))
                        weightsOfParams = lasagne.layers.get_all_param_values(network)
                        print("Angle of first image for class 0, train loss for all classes, class label")
                        print(weightsOfParams[1][targets[0]], train_loss_before[0], targets[0])
                        print(side_network_result[0])
            train_err += train_loss
            if 0:
                train_loss, transformed_image_res, predictions_res, side_network_result = eval_affine_fn(inputs, fake_targets)
                weightsOfParams = lasagne.layers.get_all_param_values(network)
                print(weightsOfParams[-1])
                print(weightsOfParams[-2])
                print(np.min(predictions_res))
                print(predictions_res.shape)
                np.save("./prediction.npy", predictions_res)
            if 0:
                np.save("./transformed_images.npy", transformed_image_res)
                np.save("./original_images.npy", inputs)
                print (np.mean(transformed_image_res[0] - inputs[0]))
                #print ("batch %d" % train_batches, "==================")
                train_err += train_model(inputs, targets, fake_targets)
                train_batches += 1
        ...
        if 0:
            ...
            for batch in iterate_minibatches(X_test, y_test, batch_size, shuffle=False):
                affine_params[0].set_value(affine_matrix_matrix)
                params[1].set_value(affine_matrix_matrix)
                inputs, targets = batch
                inputs = inputs.reshape(batch_size, 1, 40, 40)
                fake_targets = np.zeros((batch_size, 10))
                fake_targets[:, ] = np.arange(10)
                fake_targets = np.array(fake_targets.reshape((batch_size * 10,)), dtype = np.int32)
                for i in range(100):
                    train_loss, transformed_image_res, predictions_res = train_affine_fn_val(inputs, fake_targets)
                    #print(train_loss)
                    weightsOfParams = lasagne.layers.get_all_param_values(network)
