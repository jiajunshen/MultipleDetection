...
def main(model='mlp', num_epochs=20000):
    ...
    #updates = lasagne.updates.(loss, params, learning_rate = 0.001)
    ...
    train_fn = theano.function([input_var, target_var, learning_rate], [loss, prediction], updates=updates)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, y_train, 100, shuffle=True):
            ...
            result_train = train_fn(inputs, targets, current_lr)
            train_err += result_train[0]
            #print(result_train[0], np.max(result_train[1]))
            if train_batches % 10 == -1:
                print(result_train[1][:2])
                print(np.max(result_train[1]))
            train_batches += 1
        ...
        if epoch % 50 == 0:
            ...
            np.save("../data/curet_test_hinge_epoch_%d_2pool_61_classes_small.npy" %epoch, weightsOfParams)
