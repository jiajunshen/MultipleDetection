...
def load_data(trainingData, trainingLabel, testingData, testingLabel, dataset = "CURET"):
    ...
    X_train = np.array(np.load(trainingData), dtype = np.float32).reshape(-1, 1, 200, 200)
    Y_train = np.array(np.load(trainingLabel), dtype = np.uint8)
    X_test = np.array(np.load(testingData), dtype = np.float32).reshape(-1, 1, 200, 200)
