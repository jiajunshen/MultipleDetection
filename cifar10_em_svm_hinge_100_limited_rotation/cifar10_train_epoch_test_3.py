...
train_dir = './cifar10_theano_train_adam_test_3'
max_epochs = 2000
validation = False
validation_model = ''
...
def train():
    ...
    hinge_loss = lasagne.objectives.multiclass_hinge_loss(model_deformation_for_loss, target_var)
    ...
    for param, grad in zip([affine_params], d_loss_wrt_params):
        updates_affine[param] = param - 100 * grad
    ...
    weightsOfParams = np.load("../data/CIFAR_CNN_params_ROT_LATENT_LIMITED_ROTATION_50_update_epoch_300.npy")

    lasagne.layers.set_all_param_values(cnn_model, weightsOfParams)
    ...
    for epoch in xrange(400, max_epochs):
        ...
        if 1: 
            ...
            while(test_image is not None):
                ...
                affine_params.set_value(np.array(np.zeros(batch_size * 10,), dtype = np.float32))
                acc_original = val_fn(test_image, test_label)
                total_acc_count += acc * test_image.shape[0]
                total_original_acc_acount += acc_original * test_image.shape[0]
            ...
            print("  test accuracy:\t\t{:.2f} %".format(
                  float(total_original_acc_acount / total_count) * 100))
        ...
        if epoch % 50 == 0:
            ...
            while(start + batch_size != cifar10_data.train._num_examples):
                ...
                #train_affine_result = train_affine_fn(train_image)
                #print(np.mean(train_affine_result[0]))
                #print(np.mean(np.array([train_affine_result[1].reshape(-1, 10)[i, train_label[i]] for i in range(100)])))
        ...
        if 1:
            ...
            if epoch % 100 == 0:
                weightsOfParams = lasagne.layers.get_all_param_values(cnn_model)
                np.save("../data/CIFAR_CNN_params_ROT_LATENT_LIMITED_ROTATION_50_update_epoch_%d.npy" %epoch, weightsOfParams)
