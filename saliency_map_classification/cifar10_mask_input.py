...
def read_data_sets(data_dir, distortion=True, dtype=np.float32, training_num = 60000):
    global NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN
    NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = training_num
    
    all_images = np.array(np.load(os.path.join(data_dir, "real_background_new_combine.npy")).reshape(-1, 32, 32, 3),dtype=dtype)
    all_images = np.rollaxis(all_images, 3, 1)
    all_labels = np.array(np.load(os.path.join(data_dir, "real_background_new_combine_mask.npy")).reshape(-1, 32 * 32), dtype=dtype) > 0
    index = np.arange(NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN + NUM_EXAMPLES_PER_EPOCH_FOR_EVAL)
    train_index = np.arange(NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN)
    np.random.shuffle(train_index)
    train_images = all_images[train_index[:training_num]]
    train_labels = all_labels[train_index[:training_num]]
    print(train_images.shape, train_labels.shape)

    test_images = all_images[index[NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN:NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN + NUM_EXAMPLES_PER_EPOCH_FOR_EVAL]]
    test_labels = all_labels[index[NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN:NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN + NUM_EXAMPLES_PER_EPOCH_FOR_EVAL]]
