...
def build_cnn(input_var=None):
    ...
    fc1 = DenseLayer(lasagne.layers.dropout(pool3, p = 0.5), 
                     ...
                     name="fc1")
    """
    return conv1, conv1a, conv2, conv2a, conv3, fc1
    """
    ...
    #conv2_upscale=lasagne.layers.Upscale2DLayer(conv2, 2)
    #conv2a_upscale=lasagne.layers.Upscale2DLayer(conv2a, 2)
    #norm1_upscale=lasagne.layers.Upscale2DLayer(norm1, 2)
    #norm2_upscale=lasagne.layers.Upscale2DLayer(norm2, 2)
    #norm3_upscale=lasagne.layers.Upscale2DLayer(norm3, 4)

    #conv3_upscale=lasagne.layers.Upscale2DLayer(conv3, 4)
    ...
    merged_layer = lasagne.layers.ConcatLayer([norm0, 
                                               ...
                                               norm3_upscale,
                                               #conv1, conv1a,
                                               #conv2_upscale, conv2a_upscale,
                                               #conv3_upscale,
                                               #norm2_upscale, norm3_upscale,
                                               fc1_upscale
                                               #conv1, conv1a,
                                               #conv2_upscale, conv2a_upscale,
                                               #conv3_upscale, fc1_upscale
                                              ],
                                              axis=1)
    """
    merged_layer = lasagne.layers.ConcatLayer([norm0, norm1_upscale, norm2_upscale],
                                              axis=1)
    """
    """
    fc4 = DenseLayer(merged_layer, num_units=IMAGE_SIZE*IMAGE_SIZE, nonlinearity=lasagne.nonlinearities.sigmoid,
                     W=lasagne.init.GlorotUniform(), b=lasagne.init.Constant(0.1),
                     name="fc4")
    """
    fc4 = Conv2DLayer(merged_layer, num_filters=1, filter_size=(1,1),nonlinearity=lasagne.nonlinearities.sigmoid,
                      W=lasagne.init.Normal(std=0.1, mean=0.0), b=lasagne.init.Constant(0.5))
    fc4 = lasagne.layers.ReshapeLayer(fc4, (-1, IMAGE_SIZE * IMAGE_SIZE))
    #fc3 = DenseLayer(merged_layer, num_units = 500, nonlinearity=lasagne.nonlinearities.sigmoid,
    #                 W=lasagne.init.GlorotUniform(), b=lasagne.init.Constant(0.1),
    #                 name="fc3")
    #fc4 = DenseLayer(fc3, num_units=IMAGE_SIZE*IMAGE_SIZE, nonlinearity=lasagne.nonlinearities.sigmoid,
    #                 W=lasagne.init.GlorotUniform(), b=lasagne.init.Constant(0.1),
    #                 name="fc4")
