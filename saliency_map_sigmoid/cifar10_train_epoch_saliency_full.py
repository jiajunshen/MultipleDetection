...
def train():
    ...
    if 1: 
        if os.path.isfile(os.path.join(train_dir, 'latest_model.txt')):
            ...
            #final_weights = [model_weights[i] for i in range(len(model_weights) - 4)] + [current_weights[-4], current_weights[-3], current_weights[-2], current_weights[-1]]
    ...
    for epoch in range(max_steps):
        ...
        while(test_image is not None):
            ...
            #print(np.max(predicted_target), np.min(predicted_target))
